...
function verifyJWT(req, res, next) {
    ...
    if (!authHeader) {
        return res.status(401).send('Unauthorigation access');
    ...
    try {
        ...
        app.get('/appointmentOptions', async (req, res) => {
            ...
            options.forEach(option => {
                const optionBooked = alreadyBooked.filter(book => book.tretment === option.name);
        ...
        //add mongodb tempory to update price field on appointment option
        ...
        //colect specialty
        app.get('/appointmentSpecilaty', async (req, res) => {
            const query = {};
            const result = await appointmentOptionCollection.find(query).project({ name: 1 }).toArray();
            res.send(result);
        })

        //add doctor store database
